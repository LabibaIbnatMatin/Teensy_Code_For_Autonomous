...
const unsigned long ESP32_TIMEOUT = 1000; // ESP32 timeout - 1 second
...
void processESP32Control(); // ESP32 Dabble control
...
// ESP32 Dabble Control Processing Function
void processESP32Control()
{
    ...
    // Switch to velocity mode if needed (same as joystick)
    if (control_mode0 == POSITION_MODE)
    {
        odrv0.setControllerMode(CONTROL_MODE_VELOCITY_CONTROL, INPUT_MODE_VEL_RAMP);
        control_mode0 = VELOCITY_MODE;
    ...
    pumpEvents(can_intf);
    String msg = Serial4.readStringUntil('\n');
    ...
    Serial.print("ESP32: WX=");
    Serial.print(wheelX_raw);
    Serial.print(" WY=");
    Serial.print(wheelY_raw);
    Serial.print(" AX=");
    Serial.print(armX_raw);
    Serial.print(" AY=");
    Serial.print(armY_raw);
    Serial.print(" G=");
    Serial.print(ghoraVal);
    Serial.print(" Grip=");
    ...
    int wheelX_pwm = map(wheelX_raw, -7, 7, 1000, 2000);
    int wheelY_pwm = map(wheelY_raw, -7, 7, 1000, 2000);
    int armX_pwm = map(armX_raw, -7, 7, 1000, 2000);
    int armY_pwm = map(armY_raw, -7, 7, 1000, 2000);
    int ghora_pwm = map(ghoraVal, -7, 7, 1000, 2000);
    int gripper_pwm = map(gripperVal, -7, 7, 1000, 2000);
    int base_pwm = map(baseVal, -7, 7, 1000, 2000);

   
    wheel(wheelX_pwm, wheelY_pwm);
    armWheel(armX_pwm, armY_pwm);
    ghora(ghora_pwm);
    gripper(gripper_pwm);
    
    // Base control - store in chVal[9] for base_arm() function
    chVal[9] = base_pwm;
...
setup()
{
    Serial.begin(115200);
    Serial5.begin(9600);
    Serial4.begin(9600); // For ESP32 Dabble communication
    ...
    // Setup CAN bus FIRST - critical for ODrive communication
    ...
    // Setup Ethernet AFTER CAN is working - network is optional
    ...
    else
    {
        ...
        else
        {
            ...
            Udp.begin(localPort);
            Serial.print("  Ethernet link: CONNECTED (IP: ");
...
void loop()
{
    ...
    //     }
    // }
    // Process ESP32 Dabble control
   

      if(Serial4.available())
      {Serial.println("ESP32 Data Available");
       processESP32Control();
      }


    // Check for ESP32 timeout
    if (millis() - last_esp32_time > 500 && esp32_available)
    {
        // Lost ESP32 connection, stop motors
        Serial.println("ESP32: Timeout - stopping motors");
        if (odrv0_user_data.available) odrv0.setVelocity(0);
        if (odrv1_user_data.available) odrv1.setVelocity(0);
        if (odrv2_user_data.available) odrv2.setVelocity(0);
        if (odrv3_user_data.available) odrv3.setVelocity(0);
        ST.motor(1, 0); // Stop Sabertooth
